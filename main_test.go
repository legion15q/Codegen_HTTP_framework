...
func TestMyApi(t *testing.T) {
	...
	cases := []Case{
		...
		Case{ //case 1 успешный запрос - POST
			Path:   ApiUserProfile,
		...
		Case{ //case 2 сработала валидация - логин не должен быть пустым
			Path:   ApiUserProfile,
			Query:  "",
		...
		Case{ //case 3 получили ошибку общего назначения - ваш код сам подставил 500
			Path:   ApiUserProfile,
		...
		Case{ //case 4 получили специализированную ошибку - ваш код поставил статус 404 оттуда
			Path:   ApiUserProfile,
		...
		// ------
		Case{ //case 5 это должен ответить ваш ServeHTTP - если ему пришло что-то неизвестное (например когда он обрабатывает /user/)
			Path:   "/user/unknown",
			Query:  "login=not_exist_user",
			Status: http.StatusNotFound,
			Result: CR{
		...
		Case{ //case 7 юзер действительно создался
			Path:   ApiUserProfile,
		...
		Case{ //case 17 status по-умолчанию
			Path:   ApiUserCreate,
			Method: http.MethodPost,
		...
		Case{ //case 18 обрабатываем неизвестную ошибку
			Path:   ApiUserCreate,
			Method: http.MethodPost,
